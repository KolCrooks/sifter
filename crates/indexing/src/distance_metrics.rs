...
#[inline]
// hamming distance with const vector size input so that the loop can be unrolled
fn hamming_distance_const<const CHUNKS_PER_VEC: usize, ChunkT: VecChunk>(
    a: &[ChunkT],
    b: &[ChunkT],
) -> u32 {
    let mut dist = 0;
...
// hamming distance function that can handle any vector size at the cost of using a loop
fn hamming_distance_dyn<ChunkT: VecChunk>(a: &[ChunkT], b: &[ChunkT]) -> u32 {
    let mut dist = 0;
    for i in 0..a.len() {
        dist += (a[i] ^ b[i]).count_ones();
    }
    ...
    pub fn into_fn<ChunkT: VecChunk>(self, chunks_per_vec: usize) -> DistanceMetricFn<ChunkT> {
        match self {
            DistanceMetric::Hamming => {
                macro_rules! dist {
                    ($num: expr) => {
                        hamming_distance_const::<$num, ChunkT>
                    };
                }
